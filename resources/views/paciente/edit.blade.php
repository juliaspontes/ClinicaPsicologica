...
<div class="container flex-grow-1">
    <form method="POST" action="/paciente/{{$paciente->id}}">
        ...
        <div class="form-group">
            ...
            <textarea id="nome" name="nome" class="form-control" rows="1" required>
                {{ $paciente->nome }}
            </textarea>
        </div>
        <div class="form-group">
            ...
            <input type="date" id="data_nascimento"
                value='{{$paciente->data_nascimento}}' name="data_nascimento" class="form-control" required>
        </div>
        <div class="form-group">
            <label for="endereco">Endereço</label>
            <textarea id="endereco" name="endereco" class="form-control" rows="1" required>
                {{ $paciente->endereco }}
            </textarea>
        </div>
        ...
        <div class="col-md-6">
            ...
            <input type="tel" id="telefone" name="telefone" class="form-control" required value="{{ $paciente->telefone }}">
        </div>
        <div class="col-md-6">
            <label for="email">E-mail</label>
            <input type="email" id="email" name="email" class="form-control" required value="{{ $paciente->email }}">
        </div>
        </div>
...
<script>
    $(document).ready(function() {
        // Aplica a máscara de telefone (exemplo: (99) 99999-9999)
