...
<head>
    ...
    <title>Edição de Tratamento Psicológico</title>
...
<div class="header">
    <h2>Edição de Tratamento Psicológico</h2>
</div>
...
<div class="container flex-grow-1">
    <form method="POST" action="/tratamento/{{$tratamento->id}}">
        ...
        <div class="form-group">
            ...
            <select disabled id="paciente_id" name="paciente_id" class="form-control" required>
                @foreach($pacientes as $p)
                    <option value="{{ $p->id }}"
                        {{ $p->id == $tratamento->paciente_id ? 'selected' : '' }}>
                        {{ $p->nome }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            ...
            <textarea id="objetivos" name="objetivos" class="form-control" rows="3" required>
                {{ $tratamento->objetivos }}
            </textarea>
        </div>
        <div class="form-group">
            <label for="progresso">Progresso</label>
            <textarea id="progresso" name="progresso" class="form-control" rows="2" required>
