...
<head>
    ...
    <title>Edição de Prescrição Psicológica</title>
...
<div class="header">
    <h2>Edição de Prescrição Psicológica</h2>
</div>
...
<div class="container flex-grow-1">
    <form method="POST" action="/prescricao/{{$prescricao->id}}">
        ...
        <div class="form-group">
            <label for="data_prescricao">Data</label>
            <input type="date" id="data_prescricao"
                value='{{$prescricao->data_prescricao}}' name="data_prescricao" class="form-control" required>
        </div>
        <div class="form-group">
            ...
            <select disabled id="paciente_id" name="paciente_id" class="form-control" required>
                @foreach($pacientes as $p)
                    <option value="{{ $p->id }}"
                        {{ $p->id == $prescricao->paciente_id ? 'selected' : '' }}>
                        {{ $p->nome }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            <label for="descricao_prescricao">Prescrição</label>
            <textarea id="descricao_prescricao" name="descricao_prescricao" class="form-control" rows="4" required>
                {{ $prescricao->descricao_prescricao }}
            </textarea>
        </div>
